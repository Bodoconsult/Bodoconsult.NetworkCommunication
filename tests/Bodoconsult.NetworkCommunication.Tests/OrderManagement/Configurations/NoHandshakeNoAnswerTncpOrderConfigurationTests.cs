...
internal class NoHandshakeNoAnswerTncpOrderConfigurationTests
{
    ...
    public void Ctor_ValidSetup_PropsSetCorrectly()
    {
        ...
        // Assert
        Assert.That(config.ConfigurationName, Is.EqualTo($"{BuiltinOrders.NoHandshakeNoAnswerTncpOrder}Configuration"));
        Assert.That(config.OrderTypeName, Is.EqualTo(BuiltinOrders.NoHandshakeNoAnswerTncpOrder));
        Assert.That(config.OrderBuilder.GetType().Name, Is.EqualTo($"{BuiltinOrders.NoHandshakeNoAnswerTncpOrder}Builder"));
    }
}