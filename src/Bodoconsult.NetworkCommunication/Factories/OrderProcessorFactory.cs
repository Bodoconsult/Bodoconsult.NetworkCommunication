...
/// <summary>
/// Factory for creating <see cref="OrderProcessor"/> instances
/// </summary>
public class OrderProcessorFactory : IOrderProcessorFactory
{
    ...
    public OrderProcessorFactory(IAppDateService appDateService,
    ...
    {
        if (device is not INoStateMachineOrderManagementDevice noStateMachineDevice)
        {
            throw new ArgumentException("Device must be implement INoStateMachineOrderManagementDevice");
        }

        return new OrderProcessor(noStateMachineDevice, AppDateService, orderPipeline, SyncOrderManager,
