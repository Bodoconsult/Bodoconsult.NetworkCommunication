...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <html>
      <head>
        <title>Virtualisation infrastructure</title>
      ...
      <body>
        ...
        <xsl:apply-templates select="r:VirtualInfrastructure"/>
        </div>
        </div>
        <div id="RightColumn">
          <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
  ...
  <xsl:template match="r:VirtualInfrastructure">
    <h1>
      Virtualisation infrastructure
    </h1>

    <xsl:apply-templates select="r:VirtualHosts" />
   
      
  </xsl:template>


  <xsl:template match="r:VirtualHosts">
    <xsl:apply-templates select="r:VirtualHost">
      <xsl:sort select="VirtualHost/Ip" data-type="text" order="ascending" />
    </xsl:apply-templates>
  </xsl:template>
